"""
Document Processing usando PyPDF2 per convertire PDF in testo Markdown semplice
"""
import logging
...
class PdfProcessor:
    """Processore di documenti basato esclusivamente su PyPDF2"""

    def __init__(self, config: DocumentProcessingConfig):
        self.config = config
        self.logger = logging.getLogger(__name__)
        self.logger.info("ℹ️ PDF Processor attivo: uso PyPDF2 per il processing dei PDF")
    ...
    def _process_with_fallback(self, file_path: Path) -> ProcessedDocument:
        """Processa PDF/txt/md usando metodi semplici"""
    ...
    def process_directory(self, directory_path: str) -> List[ProcessedDocument]:
        ...
        self.logger.info(
            f"Processati {len(processed_docs)} documenti dalla directory {directory}"
        )
        return processed_docs


def create_processor(config: Optional[DocumentProcessingConfig] = None) -> PdfProcessor:
    """Factory function per creare un processore PDF"""
    if config is None:
