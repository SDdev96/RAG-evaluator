...
def main():
    ...
    # Test semantic chunking (solo se abbiamo OpenAI API key)
    if os.getenv("OPENAI_API_KEY"):
        chunk_ok, chunks = test_semantic_chunking(processed_doc)
        if not chunk_ok:
            print("\n⚠️ Warning: problemi con semantic chunking")
    else:
        print("\n⚠️ Skipping semantic chunking test (no OpenAI API key)")
    ...
    print(f"{'✅' if os.getenv('OPENAI_API_KEY') else '⚠️'} Semantic Chunking: {'OK' if os.getenv('OPENAI_API_KEY') else 'Skipped'}")
